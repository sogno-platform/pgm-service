...
def download_grid_data(input_data: InputData):
    eq = get_from_fileservice(input_data.eq)
    sv = get_from_fileservice(input_data.sv)
    tp = get_from_fileservice(input_data.tp)
    eq_path = f"./cim_files/{input_data.eq}_EQ.xml"
    sv_path = f"./cim_files/{input_data.sv}_SV.xml"
    tp_path = f"./cim_files/{input_data.tp}_TP.xml"
